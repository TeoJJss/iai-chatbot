...
async def reply(usr_inp):
    ...
    if [i for i in ["maybank", "cimb", "flywire","apiit", "apu", "jompay", "moodle", "bursary", "apspace", "apkey", 
                    "jaspersoft", "gims", "aws"] if i in tmp_inp.lower()]: # No autocorrect if special keywords found
        inp = tmp_inp
    else:
        inp = spell(tmp_inp)
    ...
    if str(inp) in ["hi", "hello", "greetings", "hey"]:
        ...
        if possible_answers:
            for ele in possible_answers:
                ...
                if "Next shuttle `" in ele[1] and [i for i in ['to apu', 'to apiit', 'to lrt', 'to fortune',
                                                                 'to m', 'to mosque'] if i in tmp_inp.lower()]:
                    if ele[2] > tmp_similarity:
                        quest, response, tmp_similarity = ele[0], ele[1], ele[2]
                else:
                    if (int(ele[2]) >= 2) and "Next shuttle `" not in ele[1]:
                        quest, response, tmp_similarity = ele[0], ele[1], ele[2]
                        break
                    if (int(ele[2]) >= 1):
                        if not super_high:
